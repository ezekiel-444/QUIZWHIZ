...
function generateFruits(){
    ...
    function fallDownFruit(){
        if (fruitBottom < basketBottom + 50 && fruitBottom > basketBottom && fruitLeft > basketLeft -
            30 && fruitLeft < basketLeft + 80) {
               fruits.removeChild(fruit); 
               clearInterval(fallInterval);
               score++;
            }
        if (fruitBottom < basketBottom){
            alert("game over! YOU LOSE YOU SUCKER HAHAHHAHAHA. your score is:"+ score);
            clearInterval(fallInterval);
